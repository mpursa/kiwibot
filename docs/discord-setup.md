...
The bot appears in the member list **offline**. That's expected — it comes online when the process on the VPS starts. On startup it registers its commands (`/bot`, `/bot_version`, `/list`, `/address`, `/password`, `/admin`, `/players`, `/status`, `/start`, `/stop`); guild-scoped commands appear within seconds.
